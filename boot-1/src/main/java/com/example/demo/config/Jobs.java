...
public class Jobs {
	...
	public void fixedRateJob() {
		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date()) + " >>fixedRate执行....");
	}

	// 而还有一类定时任务，比如是每天的3点15分执行, cron表达式
	/**
	 ...
	 * 第7为，年份，可以留空，取值1970-2099 (*)星号：可以理解为每的意思，每秒，每分，每天，每月，每年...
	 ...
	 * y，等同于0/y 0 0 3 * * ? 每天3点执行 0 5 3 * * ? 每天3点5分执行 0 5 3 ? *
	 * *每天3点5分执行，与上面作用相同 0 5/10 3 * * ? 每天3点的 5分，15分，25分，35分，45分，55分这几个时间点执行 0
	 * 10 3 ? * 1 每周星期天，3点10分 执行，注：1表示星期天 0 10 3 ? * 1#3 每个月的第三个星期，星期天
	 * 执行，#号只能出现在星期的位置
	 */

	@Scheduled(cron = "0 1-59 13-14 * * ?")
	public void cronJob() {
		System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date()) + " >>cron执行....");
	}
}
